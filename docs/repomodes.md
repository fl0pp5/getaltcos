...
- (Пере)генерация репозитория через `mkimage-profiles с` созданием новой мажорной версии или даты репозитория 
- Минорные обновления репозиториев  через команду `apt-get` 
...
Каждое обновление должно обеспечивать формирование новой ветки (commit) с новой версией
