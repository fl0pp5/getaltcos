# Режимы работы с ALTCOS ostree репозиторием


## Формирование потоков (`stream`), веток (`ref`), версий (`version`) ALTCOS ostree репозиториев

### Глоссарий:
- `Поток` (`stream`) - последовательной версий (commit) для определенной версии дистрибутива ALTLinux: `sisyphus`, `p10`, `p11`, .... Имя потока присутствует в ветке `altcos/x86_64/p10` и версии (`p10.20210820.2.12`) ostree-репозиторий
- `Ветка`(`ref`) - основной поток, ведущийся администратором, имеет ветку `altcos/<arch>/<stream>`:
  *  `altcos/x86_64/sisyphus`, `altcos/ppc64le/sisyphus`, ...
  *  `altcos/x86_64/p10`, `altcos/ppc64le/p10`, ...
  * ...

  Администратор или пользователи могут через REST-интерфейс создавать подветки со своим набором основных пакетов:
  *  `altcos/x86_64/Sisyphus/apache2`, `altcos/ppc64le/Sisyphus/ruby`, ...
  *  `altcos/x86_64/P10/apache2`, `altcos/ppc64le/P10/ruby`, ...
  * ...
...
### Режимы формирования ALTCOS ostree репозиториев

В настоящий момент предполагается два основных режима работы с репозиториями `ALTCOS ostree`:
...
- `sisyphus` - `altcos/x86_64/sisyphus`, `altcos/ppc64le/sisyphus`, ...
- `p10` - `altcos/x86_64/p10`, `altcos/ppc64le/p10`, ...
...
(http://ftp.altlinux.org/pub/distributions/ALTLinux/Sisyphus/, http://ftp.altlinux.org/pub/distributions/ALTLinux/p10/, ...). Это должно приводить к обновлению пакетов в репозиториях `ALTCOS ostree`.
...
Если администратором потоков принимается решение о смене состава пакетов (добавление, удаление) в репозитории, то это должно приводить к формированию версии репозитория `ALTCOS ostree` с новой мажорной версией:
`sisyphus.20210820.1.0`, `sisyphus.20210820.2.0`, ...

Если администратором потоков принимается решение о более существенных изменениях в репозитории `ALTCOS ostree`, приводящее к существенному изменению состава пакетов и их разворачиванию, то формируется новая версия (commit) с новой версией даты: `sisyphus.20210911.0.0`, `sisyphus.20220112.0.0`, ...

Генерация tar-файла через `mkimage-profiles` длительный и ресурсоемкий процесс, включающий полный пересчет sha256 сумм всех файлов и добавление файлов с новыми суммами в каталог `/objects/` репозитория.
...
и добавить изменения в новую минорную или мажорную версию `ALTCOS ostree` репозитория.

Этот подход:
- существенно уменьшит время и необходимые аппаратные ресурсы для формирования новой версии
- позволит автоматизировать логирование изменений пакетной базы на основе вывода команд `apt-get update`, `apt-get instal ...`, `apt-get remove`
- за счет малого времени формирования нового репозитория позволит его применять для формирования пользовательских веток репозитория: `altcos/x86_64/P10/Apache`, `altcos/ppc64le/P10/ruby`, ...
...
- при первоначальном создании образов и репозитория для новых потоков `p10`, `p11`
- при существенных изменениях пакетной базы, когда формирование новых версий и веток репозитория невозможно через `apt-get`.
...
После обновления через `apt-get` в репозитории формируется новая ветка с новой:
...
Так как эти действия (установка или удаление новых пакетов с созданием новой версии) будет производит пользователь по REST-запросу, необходимо максимально уменьшить время выполнения этих операций.
...
`inode` файла или каталога репозитория совпадает с `inode` развернутого и скорректированного дерева, то пересчет контрольной суммы не производится. Пересчитываются контрольные суммы только вновь созданных или откорректированных каталогов и файлов.

Таким образом, на втором этапе при корректировке файла необходимо исключить возможность его корректировки "на месте" без изменения `inode`.
...
Для устранения этой проблемы были рассмотрены два решения:
...
Недостатки:
- не создаются docker-образы. Тестировать решение можно только путем разворачивания (`ostree checkout`, `ostree deploy`) веток репозитория.
