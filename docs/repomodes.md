...
### Глоссарий:
- `Поток` (`stream`) - последовательной версий (commit) для определенной версии дистрибутива ALTLinux: `sisyphus`, `p10`, `p11`, .... Имя потока присутсвует в ветке `acos/x86_64/p10` и версии (`p10.20210820.2.12`) ostree-репозиторий
- `Ветка`(`ref`) - основной поток ведущийся администартором имеет ветку `acos/<arch>/<stream>`:
  *  `acos/x86_64/sisyphus`, `acos/ppc64le/sisyphus`, ...
  *  `acos/x86_64/p10`, `acos/ppc64le/p10`, ...
  * ...
  
  Администратор или пользователи могут через RSET-интерфйес создавать подветки со своим набором основных пакетов:
  *  `acos/x86_64/Sisyphus/apache2`, `acos/ppc64le/Sisyphus/ruby`, ...
  *  `acos/x86_64/P10/apache2`, `acos/ppc64le/P10/ruby`, ...
  * ...

- `Версия` - определенный `commit` репозитория коиторому привоена версия по формату 
...
- за счет малого времени формирования нового репозитория позволит его применить для формировния пользовательский веток репозитория: `acos/x86_64/P10/Apache`, `acos/ppc64le/P10/ruby`, ...
...
- при существенных изменениях в пакетой базы, когда формирование новых версий и веток репозитория невозможно через `apt-get`.

После (пере)генерации в репозитории формируется новая ветка с новой версийе даты `sisyphus.20210820.0.0`, `sisyphus.20210911.0.0`, `sisyphus.20220112.0.0`, ...
